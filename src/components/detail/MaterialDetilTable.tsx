                    ...
                    {materialItems.map((item, index) => {
                        ...
                        <React.Fragment key={uniqueKey}>
                            ...
                            <td className="py-3 px-4 text-center">
                              {item.internal_order ? <Check className="text-green-500 w-5 h-5 inline" /> : <X className="text-red-500 w-5 h-5 inline" />}
                            </td>
                            <td className="py-3 px-4 text-center">
                                <div className="flex justify-center space-x-2">
                                <Button variant="outline" size="icon" className="h-8 w-8" onClick={()=>router.push(`/detail/material/${item.hash_id}`)}>
                                    <Eye className="h-4 w-4" />
                                </Button>
                                {(emailDots === emailInputter) && (
                                    <>
                                      {(((status === '2010' || status === '1060') && transaction_type === '2') && form_type === 'Cash in Advance') && (
                                          <Button variant="outline" size="icon" className="h-8 w-8" onClick={()=>router.push(`/detail/material/edit/${item.hash_id}`)}>
                                      ...
                                      {((form_type === 'Disbursement' && (status === '2010' && transaction_type === '2')) || ((status === '1010' && transaction_type === '1') && form_type === 'Cash in Advance')) && (
                                        <>
                                          ...
                                          <Button variant="outline" size="icon" className="h-8 w-8 text-red-500 hover:text-red-700" onClick={() => handleDelete(item.hash_id)}>
