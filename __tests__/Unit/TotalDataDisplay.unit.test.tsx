...
describe('TotalDataDisplay Component', () => {
  ...
  test('renders correctly with total value', () => {
    render(<TotalDataDisplay total={1000} />);
    
    // Verify database icon is displayed
    expect(screen.getByTestId('database-icon')).toBeInTheDocument();
    
    // Verify total records text
    const recordsText = screen.getByText(/Total Records:/i);
    expect(recordsText).toBeInTheDocument();
    
    // Verify total value
    const formattedTotal = screen.getByText('1.000', { selector: 'span.text-blue-600' });
    expect(formattedTotal).toBeInTheDocument();
  });

  test('handles large numbers with correct formatting', () => {
    render(<TotalDataDisplay total={1234567} />);
    
    // Verify large number is formatted correctly
    const formattedTotal = screen.getByText('1.234.567', { selector: 'span.text-blue-600' });
    expect(formattedTotal).toBeInTheDocument();
  });
